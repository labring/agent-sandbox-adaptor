...
describe('SealosDevboxAdapter', () => {
  ...
  it('should retry code-server healthz before returning proxy target', async () => {
    vi.useFakeTimers();
    let healthCalls = 0;
    const fetchMock = vi.fn(async (input: string | URL | Request) => {
      if (String(input).endsWith('/code-server/abc123/healthz')) {
        healthCalls++;
        return new Response(healthCalls === 1 ? 'gateway route not found' : 'ok', {
          status: healthCalls === 1 ? 404 : 200
        });
      }
    ...
    const adapter = new SealosDevboxAdapter(CONFIG);
    const targetPromise = adapter.getProxyTarget('code-server');
