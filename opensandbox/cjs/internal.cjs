...
__export(internal_exports, {
  ...
  SandboxesAdapter: () => SandboxesAdapter,
  createExecdClient: () => createExecdClient,
  createLifecycleClient: () => createLifecycleClient
});
...
var FilesystemAdapter = class {
  ...
  async readBytes(path, opts) {
    const url = joinUrl(this.opts.baseUrl, "/files/download") + `?path=${encodeURIComponent(path)}`;
    const res = await this.fetch(url, {
      method: "GET",
      headers: {
        ...this.opts.headers ?? {},
